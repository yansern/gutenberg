...
describe( 'Links', () => {
	...
	it( 'allows Left to be pressed during creation when the toolbar is fixed to top', async () => {
		...
		let popover = await page.$( '.block-editor-link-control__popover-inner' );
		expect( popover ).not.toBeNull();

		// Escape should close the dialog still.
		await page.keyboard.press( 'Escape' );
		popover = await page.$( '.block-editor-link-control__popover-inner' );
	...
	it( 'allows use of escape key to dismiss the url popover', async () => {
		...
		// Wait for the URL field to auto-focus
		await waitForAutoFocus();
		expect( await page.$( '.block-editor-link-control__popover-inner' ) ).not.toBeNull();
		...
		await page.keyboard.press( 'Escape' );
		expect( await page.$( '.block-editor-link-control__popover-inner' ) ).toBeNull();
	...
	it( 'should contain a label when it should open in a new tab', async () => {
		...
		expect( link ).toBe( 'wordpress.org(opens in a new tab)' );
		await page.keyboard.press( 'Tab' );
		await page.keyboard.press( 'Tab' );
