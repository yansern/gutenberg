 ...
 *
 * @param array  $global_styles_branch Array repreeting a brach of the global styles object.
 ...
 */
function gutenberg_get_css_vars( $global_styles_branch, $prefix = '' ) {
	$result = array();
	foreach ( $global_styles_branch as $key => $value ) {
		$processed_key = str_replace( '/', '-', $key );
		$new_key       = ( $prefix ? $prefix . '-' : '' ) . $processed_key;

		if ( is_array( $value ) ) {
			$result = array_merge(
				$result,
				gutenberg_get_css_vars( $value, $new_key )
			);
		} else {
...
function gutenberg_enqueue_global_styles_assets() {
	$default_global_styles_path = dirname( dirname( __FILE__ ) ) . '/default-global-styles.json';
	...
	foreach (
		...
		if ( isset( $global_styles_definition['global'] ) ) {
			$css_vars = array_merge(
				$css_vars,
				gutenberg_get_css_vars( $global_styles_definition['global'], '--wp-gs' )
			);
		}
		if ( isset( $global_styles_definition['blocks'] ) ) {
			$css_vars = array_merge(
				$css_vars,
				gutenberg_get_css_vars( $global_styles_definition['blocks'], '--wp-gs' )
			);
		}
	}
...
/**
 * Adds class wp-gs to the frontend body class if the theme defines a theme.json.
 *
 * @param array $classes Existing body classes.
 * @return array The filtered array of body classes.
 */
function gutenberg_add_wp_gs_class_front_end( $classes ) {
	if ( locate_template( 'theme.json' ) ) {
...
/**
 * Adds class wp-gs to the block-editor body class if the theme defines a theme.json.
 *
...
function gutenberg_add_wp_gs_class_editor( $classes ) {
	...
	if ( $current_screen->is_block_editor() && locate_template( 'theme.json' ) ) {
